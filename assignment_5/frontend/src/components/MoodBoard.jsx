...
// What Changed? Added default assignment (= []) to catch undefined prop states gracefully
export default function MoodBoard({ moodItems = [], setMoodItems }) {
  ...
  const handleAddItem = (e) => {
    ...
    // Safe functional array initialization guard
    if (typeof setMoodItems === "function") {
      setMoodItems((prev) => [...(Array.isArray(prev) ? prev : []), newItem]);
    }
    setInputValue("");
  };
  ...
  return (
    <div>
      ...
      <div className="mood-grid">
        {/* What Changed? Added explicit Array.isArray checker inline wrap */}
        {Array.isArray(moodItems) &&
          moodItems.map((item) => (
            <div
              key={item.id}
              className="mood-card"
      ...
      {/* What Changed? Hardened the empty state selector to securely check arrays */}
      {(!Array.isArray(moodItems) || moodItems.length === 0) && (
        <p
          style={{
            color: "#555",
            fontStyle: "italic",
